...
const generateWarrior = async (warriorCore, signer, metadata) => {
	...
	const response = await axios.get(
		`https://cf3yjpujmk.execute-api.ap-south-1.amazonaws.com/prod/sign-origin?metadata=${messageHash}`
	);
	const gas = await warriorCore.connect(signer).estimateGas.generateWarrior(from, metadata, response.data.signature);
